...
#define SYS_HOSTGAME                232
#define SYS_JOINGAMEIP              233
#define SYS_LANGAMECOUNT            234
#define SYS_LANGAMEINFO             235
#define SYS_LANQUERY                236
#define SYS_JOINSTATE               237
#define SYS_LOCALIP                 238
#define SYS_REMOTEINFO              239
#define SYS_SENDREADY               240
#define SYS_SENDSTARTGAME           241
#define SYS_REMOTEREADY             242
#define SYS_ALLREMOTESREADY         243
...
//                                }
                    ...
                    case SYS_SENDREADY:
                        network_send_ready();
                        break;
                    case SYS_SENDSTARTGAME:
                        network_send_start_game();
                        break;
                    case SYS_REQUESTJOINSERVER:
                        network_request_join_server();
                        break;
...
// !!!BAD!!!
                    ...
                    case SYS_SERVERSTATISTICS:
                        i = 0;
                        break;
                    case SYS_SERVERPLAYERCOUNT:
                        i = server_player_count;
                        break;
                    ...
                    case SYS_LOCALIP:
                        ...
                        break;
                    case SYS_REMOTEINFO:
                        // j is compact index (0 to num_remote-1), k is info type
                        // k=0: IP address, k=1: is active, k=2: is ready
                        i = 0;
                        {
                            int ri, count = 0;
                            for(ri = 0; ri < MAX_REMOTE; ri++)
                            {
                                if(remote_on[ri])
                                {
                                    if(count == j)
                        ...
                        break;
                    case SYS_REMOTEREADY:
                        // j is compact index, returns ready state
                        i = 0;
                        {
                            int ri, count = 0;
                            for(ri = 0; ri < MAX_REMOTE; ri++)
                        ...
                        break;
                    case SYS_ALLREMOTESREADY:
                        // Returns TRUE if all connected remotes are ready
                        i = (num_remote > 0) ? TRUE : FALSE;
                        {
                            int ri;
                            for(ri = 0; ri < MAX_REMOTE; ri++)
                            {
                                if(remote_on[ri] && !remote_ready[ri])
                                {
                                    i = FALSE;
                                    break;
                                }
